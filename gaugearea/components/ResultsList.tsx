...
import { useAdFrequency } from '@/lib/workshopStore';
...
export const ResultsList: React.FC<ResultsListProps> = ({
  ...
  return (
    <div id="replacement-results" className="space-y-4 pb-20">
      {/* 1. TOP BANNER AD (Ideal Match Page Top) */}
      ...
      <div className="space-y-3">
        {processedResults.map((res, rank) => {
          ...
          return (
            ...
            {/* In-feed Banner or Native Ad every 3 options */}
            {(rank + 1) % adFrequency === 0 && rank < processedResults.length - 1 && (
              Math.floor((rank + 1) / adFrequency) % 2 === 1 ? (
                <NativeAd variant="results" className="my-3" />
              ) : (
                <BannerAd position="infeed" className="my-3" />
              )
            )}
          </React.Fragment>
          );
      ...
      {/* 2. BOTTOM BANNER AD */}
      <BannerAd position="bottom" className="my-3" />

      {/* 3. FLOATING BANNER AD (Docked smoothly above bottom mobile navigation) */}
